...
request.setAttribute("rowSize", rowSize);//1페이지당게시글수 --%>
 ...
 <table border="1">
 	<tr>
 	 <th>댓글 번호</th>
 	 <td>${Review_comment_Data.review.review_no}</td>
 	</tr>
 	<tr>
 	 <th>작성자명</th>
 	 <td>${Review_comment_Data.review.comm_writer.review_comment_writer_name}</td>
 	</tr>
 	<tr>
 	 <th>댓글 작성일</th>
 	 <td>
 	 <fmt:formatDate type="both" value="${Review_comment_Data.review.comm_credate}" />
 	 </td>
 	</tr>
<%--  	<tr>
 	 <th>제목</th>
 	...
 	<tr>
 	 <th>댓글 내용</th>
 	 <td><u:pre value="${Review_comment_Data.content.review_comment_content}"/></td>
 	     </tr>   
 	 ...
 	 <c:set var="pgNo" value="${(empty param.pageNo)?'1':param.pageNo}"/>                      
 	 	...
 	 	<td rowspan="4" colspan="5" style="text-align:center;">
	 	 	<a href="/review/modify.do?no=${Review_comment_Data.review.comm_no}&pageNo=${pgNo}&rowSize=${rowSize}"><button type="button">댓글수정</button></a>
	 	 	<br/>
	 	 	<a href="/review/deleter.do?no=${Review_comment_Data.review.comm_no}"><button type="button">댓글삭제</button></a>	 	 	
 	 	</c:if>
