...
import review.service.Review_ModifyRequest;
...
public class Review_Modify_Handler implements CommandHandler {

	private static final String FORM_VIEW = "view/review/review_modifyForm.jsp";
	...
	//수정처리 Service
	private Review_ModifyService modifyService = 
			new Review_ModifyService();
	...
	//(해당게시글의 정보가 출력되어있는)수정폼으로 이동-p669 38라인
		private String processForm(HttpServletRequest request, HttpServletResponse response) throws IOException {
			...
			try {
				 ...
				 *리턴유형 */  
				Review_Data reviewData = readService.getReview(no,false);
				...
				Review_ModifyRequest modReq = 
						new Review_ModifyRequest(authUser.getUserId(),
								no,
								reviewData.getReview().getReview_writer().getReview_writer_name(),
								reviewData.getReview().getReview_title(), 
								reviewData.getContent().getReview_content());
		...
		private String processSubmit(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			try {//P670 83라인
				...
				//4.View-P670 85라인
				return "/view/review/review_modifySuccess.jsp";  //리뷰 수정성공페이지
			}catch(Review_NotFoundException e) {
				response.sendError(HttpServletResponse.SC_NOT_FOUND);//404에러
				return null;
		...
		private boolean canModify(User authUser,Review_Data reviewData) {
			if(authUser == null) {
				return false;
			}
			//로그인한유저정보에서 id를 가져오기
			...
			//작성자정보에서 id를 가져오기
			String writerId = reviewData.getReview().getReview_writer().getReview_writer_id();
